        ...
        <div className="grid gap-10 lg:gap-14 lg:grid-cols-12 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col">
            ...
            <a href="#portfolio" className="group inline-flex items-center gap-2 text-[12px] tracking-[.25em] font-medium text-[#c5a572] uppercase mt-2">
              ดูผลงานของเรา
              <svg viewBox="0 0 24 24" className="w-4 h-4 transition-transform group-hover:translate-x-1" stroke="currentColor" strokeWidth={1.6} fill="none"><path d="M5 12h14M13 6l6 6-6 6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </a>
          </div>
          {/* Image column */}
  ...
  <div className="mt-16 md:mt-28 bg-[#0d141b] px-6 sm:px-10 md:px-14 py-10 md:py-12 flex flex-col md:flex-row items-stretch gap-10 md:gap-0 md:justify-between shadow-[0_0_0_1px_#121c26]">
          {stats.map((s)=> (
            <div key={s.label} className="flex flex-col items-center text-center flex-1">
              <div className="font-display font-bold text-[2.2rem] sm:text-[2.4rem] md:text-[2.6rem] leading-none text-[#d9bf8d] tracking-tight">{s.value}</div>
              <div className="mt-3 text-[11px] sm:text-[12px] tracking-[.35em] text-white/70 uppercase">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
