...
export function SiteFooter(){
  return (
    <footer aria-labelledby="footer-heading" className="mt-0 relative text-sm text-[#94a3b8] border-t border-[#141d29] bg-[#0b1118]">
      ...
      {/* Top columns */}
      <div className="relative max-w-7xl mx-auto w-full px-4 sm:px-8 pt-14 pb-10">
  <div className="grid grid-cols-12 gap-y-10 gap-x-8" role="presentation">
          {/* Brand + intro */}
          <div className="col-span-12 sm:col-span-6 lg:col-span-3 flex flex-col">
            <div className="mb-5">
              <Image src="/img/logowhite.png" alt="โลโก้ พรีเมียมกันสาด" width={200} height={60} className="h-10 sm:h-12 w-auto" sizes="(max-width:640px) 160px, 200px" />
            </div>
            <p className="text-[13px] leading-relaxed pr-4 opacity-90 max-w-xs text-[#b4c1d1]">ผู้เชี่ยวชาญด้านกันสาด ออกแบบ-ผลิต-ติดตั้ง ครบวงจร เน้นคุณภาพ วัสดุแท้ และดีไซน์ที่กลมกลืนกับอาคาร</p>
          </div>
          {/* Menu */}
        ...
        {/* Contact bar */}
    <div className="mt-12 flex flex-col gap-6 border-y border-[#1e2a37] py-6">
          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-10">
            <div className="flex items-center gap-4">
      <div className="flex items-center justify-center w-11 h-11 rounded-full bg-[#141e29] border border-[#1f2d3b] text-[#c5a572] shadow-inner">
                <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
              </div>
      <div className="text-[#e2e8f0] font-semibold text-xl sm:text-2xl tracking-wide">081-234-5678</div>
      <div className="hidden lg:block h-8 w-px bg-[#1e2a37] ml-4" />
        ...
        {/* Copyright */}
        <div className="pt-6 text-center text-[12px] tracking-wide text-[#647587]">
          <p>© {new Date().getFullYear()} พรีเมียมกันสาด. สงวนสิทธิ์ทั้งหมด</p>
          <p className="mt-1 text-[10px] uppercase tracking-[.25em] text-[#4f5a64]">PREMIUM AWNING • DESIGN • INSTALLATION • SERVICE</p>
        </div>
...
function SocialLink({ href, label, svg }: { href: string; label: string; svg: React.ReactNode; }) {
  return (
    <a href={href} aria-label={label} className="w-10 h-10 rounded-lg border border-[#1f2d3b] bg-[#141e29] flex items-center justify-center text-[#9fb0bf] hover:text-[#c5a572] hover:border-[#c5a572] hover:bg-[#1d2935] transition-all duration-300 hover:-translate-y-0.5 shadow-[0_0_0_1px_#1a242e,0_4px_12px_-6px_rgba(0,0,0,.5)]">
          ...
          return (
            <li key={key}>
              <a
                ...
                className="inline-block py-1 pr-2 hover:text-[#c5a572] transition-colors duration-200 hover:translate-x-1"
              >
                {it.label}
              </a>
            </li>
